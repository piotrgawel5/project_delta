...
function normalizeSleepLogBody(body: SleepRequestBody): SleepLog {
  ...
  const normalized = cleanUndefined({
    ...
    duration_minutes: (body.duration_minutes ?? body.durationMinutes) as
      | number
      | undefined,
    quality_score: (body.quality_score ?? body.qualityScore) as
      | number
      | undefined,
    deep_sleep_minutes: (body.deep_sleep_minutes ?? body.deepSleepMinutes) as
      | number
      | undefined,
    rem_sleep_minutes: (body.rem_sleep_minutes ?? body.remSleepMinutes) as
      | number
      | undefined,
    light_sleep_minutes: (body.light_sleep_minutes ??
      body.lightSleepMinutes) as number | undefined,
    awake_minutes: (body.awake_minutes ?? body.awakeSleepMinutes) as
      | number
      | undefined,
    ...
    estimated_bedtime: (body.estimated_bedtime ?? body.estimatedBedtime) as
      | string
      | undefined,
    estimated_wakeup: (body.estimated_wakeup ?? body.estimatedWakeup) as
      | string
      | undefined,
    screen_time_summary: screenTimeSummary
      ? cleanUndefined({
          total_minutes: (screenTimeSummary.total_minutes ??
            screenTimeSummary.totalMinutesLast2Hours) as number | undefined,
          last_unlock_before_bedtime:
            screenTimeSummary.last_unlock_before_bedtime as
              | string
              | null
              | undefined,
          first_unlock_after_wakeup:
            screenTimeSummary.first_unlock_after_wakeup as
              | string
              | null
              | undefined,
    ...
    data_source: (body.data_source ?? body.dataSource) as string | undefined,
    synced_at: (body.synced_at ?? body.syncedAt) as string | undefined,
    heart_rate_avg: (body.heart_rate_avg ?? body.heartRateAvg) as
      | number
      | undefined,
