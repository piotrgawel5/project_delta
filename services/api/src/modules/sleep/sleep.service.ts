...
export class SleepService {
  ...
  async saveLog(sleepLog: SleepLog): Promise<SleepDataRow> {
    ...
    if (error) {
      throw AppError.internal(`Failed to save sleep log: ${error.message}`);
    }
  ...
  async getLogByDate(
    ...
    if (error && error.code !== 'PGRST116') {
      throw AppError.internal(`Failed to fetch sleep log: ${error.message}`);
    }
  ...
  async editLog(
    ...
    if (error) {
      throw AppError.internal(`Failed to edit sleep log: ${error.message}`);
    }
  ...
  async deleteLog(userId: string, date: string): Promise<boolean> {
    ...
    if (error) {
      throw AppError.internal(`Failed to delete sleep log: ${error.message}`);
    }
