...
export async function setUploadConsent(consent: boolean): Promise<void> {
  try {
    await AsyncStorage.setItem(CACHE_KEYS.UPLOAD_CONSENT, consent ? 'true' : 'false');
...
export async function saveToCache(records: CachedSleepRecord[]): Promise<void> {
  try {
    await AsyncStorage.setItem(CACHE_KEYS.SLEEP_DATA, JSON.stringify(records));
...
export async function upsertCacheRecord(
  ...
  try {
    ...
    const index = existing.findIndex((r) => r.user_id === record.user_id && r.date === record.date);
    ...
    retentionDate.setDate(retentionDate.getDate() - RAW_DATA_RETENTION_DAYS);
    const filtered = existing.filter((r) => new Date(r.date) >= retentionDate);
...
export async function getPendingSyncRecords(): Promise<CachedSleepRecord[]> {
  try {
    ...
    console.error('[SleepCache] Error getting pending sync records:', error);
    return [];
  }
}
...
export async function updateRecordWithEdit(
  ...
  try {
    const all = await loadFromCache();
    const index = all.findIndex((r) => r.date === date && r.user_id === userId);
...
function getRecordPriority(record: Partial<CachedSleepRecord>): number {
  ...
  if (record.data_source === 'manual_entry' || record.source === 'manual') {
    return 100;
  }
  // HC with high confidence
  if (
    (record.data_source === 'health_connect' || record.source === 'health_connect') &&
...
export async function migrateCache(): Promise<void> {
  try {
    ...
    const updated = records.map((record) => {
      ...
      if (!record.source) {
        migrated = true;
        record.source = (record.data_source as DataSource) || 'health_connect';
      }
      ...
      if (!record.confidence) {
        ...
        record.confidence = record.data_source === 'manual_entry' ? 'low' : 'medium';
      }
 ...
 */
export async function getHistoricalDurations(userId: string, days: number = 7): Promise<number[]> {
  try {
    const records = await loadFromCache();
    return records
    ...
    console.error('[SleepCache] Error getting historical durations:', error);
    return [];
  }
}
