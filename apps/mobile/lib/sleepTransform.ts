...
export function transformToHypnogramStages(
  ...
  if (!data || !data.start_time || !data.end_time || data.duration_minutes <= 0) {
    return [];
  }
  ...
  const deepMins = data.deep_sleep_minutes || Math.round(totalDuration * 0.18);
  const remMins = data.rem_sleep_minutes || Math.round(totalDuration * 0.22);
  const lightMins = data.light_sleep_minutes || Math.round(totalDuration * 0.55);
