...
export function calculateLegacySleepScore(
  ...
  if (durationMinutes >= optimalDurationMin && durationMinutes <= optimalDurationMax) {
    // Perfect range
    ...
    durationScore = Math.round(SCORE_WEIGHTS.DURATION * (1 - penalty * 0.3));
  }
  ...
  if (deepPct >= OPTIMAL_TARGETS.DEEP_MIN_PCT && deepPct <= OPTIMAL_TARGETS.DEEP_MAX_PCT) {
    // Within optimal range: grade by proximity to ideal (not flat max)
    const denom =
      deepPct <= OPTIMAL_TARGETS.DEEP_IDEAL_PCT
        ? OPTIMAL_TARGETS.DEEP_IDEAL_PCT - OPTIMAL_TARGETS.DEEP_MIN_PCT
        : OPTIMAL_TARGETS.DEEP_MAX_PCT - OPTIMAL_TARGETS.DEEP_IDEAL_PCT;
    const proximity =
      denom > 0 ? 1 - Math.abs(deepPct - OPTIMAL_TARGETS.DEEP_IDEAL_PCT) / denom : 1;
    deepScore = Math.round(SCORE_WEIGHTS.DEEP_SLEEP * (0.7 + 0.3 * clamp(proximity, 0, 1)));
  ...
  if (remPct >= OPTIMAL_TARGETS.REM_MIN_PCT && remPct <= OPTIMAL_TARGETS.REM_MAX_PCT) {
    const denom =
      remPct <= OPTIMAL_TARGETS.REM_IDEAL_PCT
        ? OPTIMAL_TARGETS.REM_IDEAL_PCT - OPTIMAL_TARGETS.REM_MIN_PCT
        : OPTIMAL_TARGETS.REM_MAX_PCT - OPTIMAL_TARGETS.REM_IDEAL_PCT;
    const proximity = denom > 0 ? 1 - Math.abs(remPct - OPTIMAL_TARGETS.REM_IDEAL_PCT) / denom : 1;
    remScore = Math.round(SCORE_WEIGHTS.REM_SLEEP * (0.7 + 0.3 * clamp(proximity, 0, 1)));
  ...
  if (awakePct <= 0.05) {
    ...
    efficiencyScore = Math.round(SCORE_WEIGHTS.EFFICIENCY * (0.7 + 0.3 * ratio));
  } else {
    // Poor: > 10% awake time
    const excess = awakePct - OPTIMAL_TARGETS.AWAKE_MAX_PCT;
    const penalty = Math.min(1, excess / 0.15);
    efficiencyScore = Math.round(SCORE_WEIGHTS.EFFICIENCY * 0.7 * (1 - penalty));
  }

  // 5. Consistency score (0-10 points)
  let consistencyScore: number;
  if (historicalDurations.length >= 3) {
    const stdDev = calculateStdDev([...historicalDurations, durationMinutes]);
    if (stdDev <= 30) {
      ...
      consistencyScore = Math.round(SCORE_WEIGHTS.CONSISTENCY * Math.max(0, 1 - excess / 60));
    }
  ...
  const shortDurationPenalty = durationMinutes < 360 ? Math.round((360 - durationMinutes) / 3) : 0;
  const total =
    durationScore +
    deepScore +
    remScore +
...
export function determineConfidence(
  ...
  const hasCompleteStages = totalStages > 0 && stages.deep > 0 && stages.rem > 0;

  // Health Connect or wearable with complete data = high
  if ((source === 'health_connect' || source === 'wearable') && hasCompleteStages) {
...
export function generateExplainableInsight(
  ...
  if (duration_minutes < OPTIMAL_TARGETS.DURATION_MIN) {
    ...
    signals.push({
      ...
      description: `${Math.round(deficit / 60)}h ${deficit % 60}m below target`,
    });
  ...
  if (deepPct < OPTIMAL_TARGETS.DEEP_MIN_PCT) {
    signals.push({
      ...
      impact: -Math.round((1 - deepPct / OPTIMAL_TARGETS.DEEP_MIN_PCT) * 50),
      description: `${Math.round(deepPct * 100)}% deep sleep (target: 15-25%)`,
    });
  } else if (deepPct >= OPTIMAL_TARGETS.DEEP_MAX_PCT) {
    signals.push({
      signal: 'Excellent deep sleep',
      impact: 15,
      description: `${Math.round(deepPct * 100)}% deep sleep indicates good physical recovery`,
    });
  }
  ...
  if (remPct < OPTIMAL_TARGETS.REM_MIN_PCT) {
    signals.push({
      ...
      impact: -Math.round((1 - remPct / OPTIMAL_TARGETS.REM_MIN_PCT) * 40),
  ...
  if (awakePct > OPTIMAL_TARGETS.AWAKE_MAX_PCT) {
    signals.push({
      ...
      impact: -Math.round((awakePct - OPTIMAL_TARGETS.AWAKE_MAX_PCT) * 200),
      description: `${Math.round(awakePct * 100)}% time awake (target: <10%)`,
    });
  }

  // Screen time analysis (if available)
  if (record.screen_time_summary?.last_unlock_before_bedtime) {
    const unlockTime = new Date(record.screen_time_summary.last_unlock_before_bedtime);
    const bedtime = new Date(record.start_time);
    const minutesBefore = (bedtime.getTime() - unlockTime.getTime()) / 60000;

    if (minutesBefore < 30) {
      signals.push({
        signal: 'Late screen time',
        impact: -15,
        description: 'Screen activity within 30 minutes of bedtime may affect sleep quality',
      });
    }
  }
  ...
  if (score_breakdown.total >= 85) {
    ...
    message = 'Your sleep quality was optimal. Maintain your current sleep schedule.';
    ...
    if (topIssue.signal.includes('duration')) {
      ...
      message = 'Try avoiding alcohol and heavy meals 3 hours before bed for better deep sleep.';
    ...
    } else if (topIssue.signal.includes('WASO') || topIssue.signal.includes('awake')) {
      title = 'Sleep Fragmentation';
      message = 'Reduce disturbances by keeping your room cool (65-68°F) and dark.';
    } else if (topIssue.signal.includes('screen')) {
      title = 'Digital Wind-Down';
      message = 'Try putting devices away 30 minutes before bed for better sleep onset.';
    } else {
    ...
    message = 'Your sleep is on track. Small improvements in consistency could boost your score.';
  }
 ...
 */
function getActivityMultiplier(activityLevel: ActivityLevel | undefined): number {
 ...
 */
function getGoalAdjustments(goal: Goal | undefined): { deepAdj: number; remAdj: number } {
...
export function estimateSleepStages(
  ...
  remPercent = clamp(remPercent, 0.1, 0.35);
  lightPercent = clamp(lightPercent, 0.3, 0.65);
...
export function calculatePersonalizedQuality(
  ...
  return {
    durationScore: Math.round((breakdown.duration_norm / SCORE_WEIGHTS.DURATION) * 100),
    deepScore: Math.round((breakdown.deep_pct / SCORE_WEIGHTS.DEEP_SLEEP) * 100),
    remScore: Math.round((breakdown.rem_pct / SCORE_WEIGHTS.REM_SLEEP) * 100),
    consistencyScore: Math.round((breakdown.consistency / SCORE_WEIGHTS.CONSISTENCY) * 100),
...
export function calculateCircadianAlignment(
  ...
  // Average offsets
  const avgBedOffset = bedtimeOffsets.reduce((a, b) => a + b, 0) / bedtimeOffsets.length;
  const avgWakeOffset = wakeTimeOffsets.reduce((a, b) => a + b, 0) / wakeTimeOffsets.length;
  const avgMidpointOffset = midpointOffsets.reduce((a, b) => a + b, 0) / midpointOffsets.length;
  ...
  const totalScore = Math.round((bedScore + wakeScore + midScore + consistencyScore / 4) * 10) / 10;

  // Generate recommendation
  let recommendation = '';
  if (totalScore >= 8) {
    recommendation = 'Excellent circadian alignment! Keep maintaining your consistent schedule.';
  } else if (totalScore >= 6) {
    recommendation = 'Good alignment. Try to go to bed within 30 minutes of your target time.';
  } else if (avgBedOffset > avgWakeOffset) {
    recommendation = `Consider moving bedtime ${
      Math.round(avgBedOffset / 15) * 15
    } minutes earlier.`;
  } else {
    recommendation = `Try to wake up closer to ${idealWakeTime} for better rhythm.`;
  }
...
export function calculateRollingSleepDebt(
  ...
  if (totalDebt <= 60) {
    ...
    debtLevel = 'mild';
    recommendation = 'Minor sleep debt. Try to get an extra 30 minutes tonight.';
  } else if (totalDebt <= 360) {
    debtLevel = 'moderate';
    needsRecovery = true;
    recommendation =
      "You've accumulated ~" +
      Math.round(totalDebt / 60) +
      ' hours of sleep debt. Consider a recovery nap.';
  } else {
    debtLevel = 'severe';
    needsRecovery = true;
    recommendation = 'High sleep debt detected. Prioritize getting extra sleep this weekend.';
  }
...
export function generateBedtimeCoachingPlan(
  ...
  for (let i = 0; i < 5; i++) {
    ...
    const bedtime = `${hour.toString().padStart(2, '0')}:${min.toString().padStart(2, '0')}`;
    ...
    const formatTime = (m: number) => {
      ...
      return `${Math.floor(adj / 60)
        .toString()
        .padStart(2, '0')}:${(adj % 60).toString().padStart(2, '0')}`;
    };
  ...
  if (totalShift >= 60) {
    ...
    estimatedBenefit = 'Fine-tuning your schedule will help optimize your natural sleep rhythm.';
  }
...
export function shouldOfferBedtimePlan(
  ...
  if (circadianScore < 6) {
    return {
      shouldOffer: true,
      reason: 'Your circadian alignment could use improvement. A gradual bedtime shift can help.',
    };
  }

  // Offer if moderate/severe sleep debt
  if (sleepDebt.debtLevel === 'moderate' || sleepDebt.debtLevel === 'severe') {
    return {
      shouldOffer: true,
      reason: "You've accumulated sleep debt. An earlier bedtime can help recover.",
    };
  }
  ...
  if (hour >= 0 && hour < 6) {
    return {
      shouldOffer: true,
      reason: "Going to bed after midnight can affect sleep quality. Let's adjust gradually.",
    };
  }
