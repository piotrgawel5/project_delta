    ...
    describe("Mock Screen Events Processing", () => {
        it("should identify sleep gap from screen off events", () => {
            ...
            for (let i = 0; i < mockEvents.length - 1; i++) {
                if (
                    mockEvents[i].type === "SCREEN_OFF" &&
                    mockEvents[i + 1].type === "SCREEN_ON"
                ) {
                    const gap = mockEvents[i + 1].timestamp -
                        mockEvents[i].timestamp;
                    if (gap > longestGap) {
                        longestGap = gap;
        ...
        it("should handle fragmented sleep patterns", () => {
            ...
            for (let i = 0; i < mockEvents.length - 1; i++) {
                if (
                    ...
                    gaps.push({
                        duration: mockEvents[i + 1].timestamp -
                            mockEvents[i].timestamp,
...
describe("Screen Time Summary", () => {
    ...
    it("should identify pre-bedtime screen usage", () => {
        ...
        const minutesBeforeBed =
            (estimatedBedtime.getTime() - lastUnlock.getTime()) / 60000;
