 ...
 * @param endTime - ISO 8601 timestamp for range end
 */
export async function getSleepMetrics(startTime: string, endTime: string): Promise<SleepMetrics> {
  if (Platform.OS !== 'android') {
    throw new Error('Health Connect is only available on Android');
  }
...
export async function getLastNightSleep(): Promise<SleepSession | null> {
  ...
  const sessions = await getSleepSessions(yesterday.toISOString(), now.toISOString());
  ...
  return sessions.sort((a, b) => new Date(b.endTime).getTime() - new Date(a.endTime).getTime())[0];
}
...
export function calculateSleepQuality(session: SleepSession): number {
  ...
  if (deepRatio >= 0.15 && deepRatio <= 0.3) {
    score += 20;
